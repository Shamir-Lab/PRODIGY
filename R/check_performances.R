...
check_performances<-function(ranked_genes_lists,snv_matrix,gold_standard_drivers)
{
	...
	f1_matrices = list()
	precision_matrix = matrix(ncol=20)
	recall_matrix = matrix(ncol=20)
	f1_matrix = matrix(ncol=20)	
	for(i in 1:length(ranked_genes_lists))
	{
		...
		if(length(intersect(gold_standard_drivers,ranked_genes_lists[[i]])) < 1)
		{
			precision_matrix = rbind(precision_matrices,rep(0,20))
			recall_matrix = rbind(recall_matrices,rep(0,20))
			f1_matrix = rbind(f1_matrices,rep(0,20))
			next
		}
			precision_matrix = rbind(precision_matrix,calculate_precision(ranked_genes_lists[[i]][1:min(20,length(ranked_genes_lists[[i]]))],gold_standard_drivers)[1:20])
			recall_matrix = rbind(recall_matrix,calculate_recall(ranked_genes_lists[[i]][1:min(20,length(ranked_genes_lists[[i]]))],intersect(gold_standard_drivers,patient_snp))[1:20])
			curr_f1 = (2*precision_matrix[nrow(precision_matrix),]*recall_matrix[nrow(recall_matrix),])/(precision_matrix[nrow(precision_matrix),]+recall_matrix[nrow(recall_matrix),])
			curr_f1[is.nan(curr_f1)] = 0
			f1_matrix = rbind(f1_matrix,curr_f1)		
	}
	precision_matrix = precision_matrix[-1,]
	recall_matrix = recall_matrix[-1,]
	f1_matrix = f1_matrix[-1,]		
